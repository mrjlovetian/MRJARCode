...
@interface YHJQRCodeScanningView ()
...
@implementation YHJQRCodeScanningView
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    UIImage *left_image = [UIImage imageNamed:@"YHJQRCode.bundle/YHJCodeLeftTop"];
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:@"YHJQRCode.bundle/YHJCodeRightTop"];
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"YHJQRCode.bundle/YHJCodeLeftBottom"];
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"YHJQRCode.bundle/YHJCodeRightBottom"];
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.image = [UIImage imageNamed:@"YHJQRCode.bundle/YHJCodeScanningLine"];
...
- (void)addTimer {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:YHJQRCodeScanningLineAnimation target:self selector:@selector(timeAction) userInfo:nil repeats:YES];
...
- (void)timeAction {
    ...
    if (flag) {
        ...
        flag = NO;
        [UIView animateWithDuration:YHJQRCodeScanningLineAnimation animations:^{
            frame.origin.y += 5;
